...
During the last 20 years methods have been developed for the purpose of
estimating a case's deviancy compared to a control group, while controlling Type
I errors, mainly by John Crawford and Paul Garthwaite [e.g.,
@crawfordComparingIndividualTest1998; @crawfordComparingSingleCase2011;
@crawfordComparisonSingleCase2007; @crawfordInvestigationSingleCase2002;
@crawfordTestingSuspectedImpairments2005]. Their most popular methods focus on
i) estimating abnormality of a case compared to a control sample on a single
variate and ii) estimating abnormality of a case compared to a control sample on
the \emph{discrepancy} between two variates.
...
a single variate will be referred to as a deficit, and an abnormally large
discrepancy between two variates will be referred to as a dissociation. In a
neuropsychological context, the ability to test for a dissociation is often
considered to be of cardinal importance. This is because dissociations provide
evidence for functional independence between two cognitive abilities. By
charting dissociations, a cognitive architecture of the mind can be theorized
[@shalliceNeuropsychologyMentalStructure1988].

The methods devised by Crawford and colleagues have not yet been implemented in
any standard statistical environment. By doing so in the package \pkg{singcar} for the \proglang{R}
environment [@rcoreteamLanguageEnvironmentStatistical2020], our aim is to encourage and simplify their
usage. Further, limiting Type II errors has not received as much attention as
limiting Type I errors in single-case methodology [@mcintoshPowerCalculationsSinglecase2020]. By
including novel tools for power calculations, our hope is to increase awareness
of the inherently low power in this field as well as to aid in the planning and
designing of experiments.

The \proglang{R} package \pkg{singcar} contains 7 functions to estimate a case's
deviancy to a normal population estimated from a small sample, 3 with regards to
a single variate and 4 with regards to the discrepancy between two variates.
...
Of special note for psychological research are the more-recently developed
methods allowing the inclusion of covariates [@crawfordComparingSingleCase2011],
using Bayesian regression modelling. These methods makes matching the control
...
dataset from a recent single case study in neuropsychology.
...
neuropsychological researchers to compare single cases against a control
...
a restricted control sample as if they were population parameters. This would possibly
be justifiable with very large samples, but
...
probable than overestimation, inflating obtained \(Z\) scores and thus, if used for
hypothesis testing, Type I errors \citep{crawfordComparingIndividualTest1998}.

The package \pkg{singcar} implements a range of methods that have been devised to
...
An appropriate approach for comparing a single observation to the mean of a
...
unstandardised difference test (UDT) is done in
an analogous way to that of the confidence intervals of TD \citep{crawfordTestingSuspectedImpairments2005}. However, the test
is somewhat limited in its applicability. Far more common, at least in neuropsychology, is the need to
assess discrepancies between differently-scaled variates, which require standardisation to be comparable.

By standardising the variates (not taking sample size into account)
we get an effect size measure similar to \(Z_{CC}\), Equation \eqref{eq:zcc} \citep{crawfordPointIntervalEstimates2010}:
\begin{equation}
Z_{DCC}  = \frac{z^*_1 - z^*_2}{\sqrt{2-2\rho_{12}}}
\label{eq:PJ}
\end{equation}
Where \(z_1^*\) and \(z_2^*\) are the standardised case scores on a task A and a task B
and the subscript (DCC) indicates ``discrepancy-case-controls'' comparison. This quantity
was proposed by \citet{payneStatisticsInvestigationIndividual1957} as a significance test for discrepancies,
but is of course not appropriate for such a purpose if small samples are used.
This is due to the fact that we cannot use the \(Z\) distribution for estimating the
abnormality of the discrepancy for the same reason we cannot use \(Z_{CC}\) to test for
a deficit. But also because we cannot use use \(Z\) scores to represent the case's scores
...
demonstrate that \(\mathbb{P}[ \psi > c] \approx \mathbb{P}[t >c]\). To obtain a
...
Where \(p = \mathbb{P}[t_{n-1}>c]\) is the estimate of abnormality and used for
significance testing. It should be noted that because \(c\) is quadratic and we
choose the positive root, the resultant statistic cannot be negative. Therefore,
if it is required that the test statistic expresses the direction of a negative
effect the correct sign would have to be imposed.

The quantity in Equation \eqref{eq:RSDT} is referred to as the ``revised standardised
difference test'' \citep[RSDT:][]{crawfordTestingSuspectedImpairments2005}. The name stems from it being
preceded by a test similar to that of Equation \eqref{eq:UDT} but with standardised
variates, developed by \citet{crawfordPayneJonesRevisited1998} and that of \citet{payneStatisticsInvestigationIndividual1957}.
\citet{crawfordTestingSuspectedImpairments2005} show with Monte Carlo simulations that the RSDT is
superior to both their own previous test and \citet{payneStatisticsInvestigationIndividual1957}'s in
controlling Type I errors.

Even for very small sample sizes of \(n=5\), RSDT was shown to barely exceed the
specified 5\% error rate. However, if a case lacks significant discrepancy
between the variates but exhibits extreme scores on both of them, the error rate
of the RSDT increases steeply with the extremity of the scores.
\citet{crawfordComparisonSingleCase2007} showed that the RSDT starts to lose control of the
error for task scores being more extreme than two standard deviations away
from the mean on both variates, without them exhibiting a discrepancy. For some
parameter combinations error rates as large as 34\% was shown.
...
are treated as fixed attributes of a population, estimations of which in a
series of trials will converge to the true value. In the Bayesian
framework, by contrast, parameters are treated as random variables with
associated probability distributions. To estimate a parameter distribution, we
can use any prior knowledge of that parameter to assign
probabilities to possible values of the parameter, forming what is known as a
prior distribution, or simply a prior. If no prior information is available
we may want to use a non-informative prior, the most simple of which would assign
equal probabilities to all possible parameter values (uniform prior). The prior is
updated when new information is obtained to form what is called a posterior
distribution. The posterior
probability of a hypothesis (i.e., a specified value of the parameter) is
calculated by using Bayes theorem. If we disregard the marginal probability of
the data in Bayes theorem it can be rewritten as:
\begin{equation*}
...
prior density of the hypothesis.

In many cases, however, it is not feasible to calculate the posterior
analytically. Instead one may use Markov Chain Monte Carlo (MCMC) methods. A
Markov Chain describes a series of linked events where the probability for any
of them to occur is contingent on the outcome of the preceding event. Monte
Carlo methods are mathematical algorithms that solve numerical problems by
repeated sampling from distributions. The idea is that one estimates the
posterior by drawing a large number of random samples from the combination of
the likelihood and prior, thus ``constructing'' it. The methods for drawing these
samples differ depending on the type of distribution and problem at hand. But in
general they are all building on algorithmic rules of drawing random numbers
based on the likelihood and the prior, and observing the distribution after a
large number of iterations.

The peak of such a distribution is often taken as an estimation of the parameter of
interest and when using non-informative priors this often corresponds to the
maximum likelihood estimate of the parameter. Hence, using a non-informative
prior often yields estimations with frequentist properties, a feature that is
useful if hypothesis testing is desired.
...
following sections describe the procedural details of these Bayesian methods.
The steps presented here closely follow those outlined in the original papers
with just a few slight changes of notation. The methods are all based on Monte
Carlo simulations but Markov Chains are run for only a subset of the problems.
...
\(\mu|\sigma^2 \sim \mathcal{N}(\overline{y}, \sigma^2/n)\).
...
\item
  ...
  case score then \(\hat{p}_{(i)} =\mathbb{P}[Z < z^*_{(i)}]\) or \(\hat{p}_{(i)} = 1-\mathbb{P}[Z < z^*_{(i)}]\)
  depending on alternative hypothesis, is the estimate of \(p\) for this
  iteration. That is the probability of drawing a
  value less than \(z^*_{(i)}\) from a standard normal distribution.
\end{enumerate}
...
show that this method yields converging results to that of the frequentist TD, Equation \eqref{eq:TD}.

### The Bayesian standardised difference test {#BSDT}

The Bayesian standardised difference test (BSDT) follow a similar procedure to that of BTD, however, we now assume a
sample of \(n\) controls from which we obtain values on the variates \(Y_1\)
and \(Y_2\) following a bivariate normal distribution and representing task
A and task B. Let \(\overline{y}_1\) and \(\overline{y}_2\) denote the sample means
and
\begin{equation*}
\pmb{A}=\begin{bmatrix}
s^2_{1} & s_{12} \\
...
That is, a Wishart distribution of 1 dimension is a \(\chi^2\)-distribution on \(n\) degrees of
freedom. The Wishart distribution is the conjugate prior of \(\Sigma^{-1}\) and the inverse
Wishart distribution is the conjugate prior of \(\Sigma\). One can view the Wishart distribution
parameterised with \(n\) degrees of freedom and some variance-covariance matrix as being a distribution
of SSCP-matrices related to that variance-covariance matrix. Similarly, one can view the
inverse Wishart distribution parameterised with the same degrees of freedom and a SSCP-matrix
as being a distribution of variance-covariance matrices related to that SSCP-matrix.

The joint prior for \(f(\pmb\mu, \Sigma^{-1})\) chosen in
\citet{crawfordComparisonSingleCase2007} (note the inverse of \(\Sigma\)) was \(f(\pmb\mu, \Sigma^{-1}) \propto |\Sigma|\). The posterior marginal distribution of
\(\Sigma^{-1}\) for this choice of prior is a Wishart distribution with \(n\)
degrees of freedom and scale matrix \(\pmb{S}^{-1}\). To sample from the posterior
of \(\Sigma\) we thus sample from an inverse Wishart distribution on \(n\) degrees
of freedom and scale matrix \(\pmb{S}\). The conditional distribution of
\(\pmb\mu|\Sigma\) is then a multivariate normal distribution with mean
\([\overline{y}_1 \ \overline{y}_2]\) and variance \(\frac{1}{n}\Sigma\), \citep[see
e.g.,][]{gelmanBayesianDataAnalysis2013}.

\citet{crawfordComparisonSingleCase2007} refer to this as the "standard theory"\footnote{Independent
Jeffrey's prior.} prior. Use of the standard theory prior produces good frequentist properties
for \(\sigma_1\) and \(\sigma_2\), but \citet{bergerObjectivePriorsBivariate2008} have shown that the
convergence to frequentist estimates is less good for \(\rho\)
and discrepancy sizes \citep{crawfordComparingSingleCase2011}. \citet{bergerObjectivePriorsBivariate2008} instead recommend that
\(f(\pmb\mu, \Sigma^{-1}) \propto \frac{1}{\sigma_1\sigma_2(1-\rho^2)}\)
should be used as a general purpose prior.
...
improved. \citet{crawfordComparingSingleCase2011} recommends this and refer to it as the
...
1.  Let \(z_{r1}\) and \(z_{r2}\) be two random draws from a standard normal distribution.
    ...
    z_{r1} \\
    z_{r2} \end{pmatrix} \cdot \frac{1}{n}
    \end{equation*}
...
developing Bayesian regression methods that allowed the case's abnormality to be assessed
...
bracket the case on covariates of interest, in order to avoid extrapolating `out
...
no assumption is made about the distribution of the covariates.
...
is a \(k\)-variate normal distribution with mean vector \(\boldsymbol{B}^*_{\text{vec}}\)
...
They are similar to \(Z_{CC}\) \eqref{eq:zcc} and \(Z_{DCC}\) \eqref{eq:PJ}, however,
the extra \(C\) in the subscript indicates that they are conditional on
covariates. Denote \(y^*_1\) and \(y^*_2\) as the case's scores on two variates
...
We then find the tail area under the standard normal density that is less or greater than
...
<!-- The main functions of \pkg{singcar} are: -->

<!-- Name | Function | Source | Reference  -->
<!-- ---------------|----------|-------|-------- -->
<!-- Test of deficit |\code{TD()} | @crawford_comparing_1998 | Equation \@ref(eq:TD) -->
<!-- Bayesian test of deficit | \code{BTD()} | @crawford_comparison_2007 | Section \@ref(BTD) -->
<!-- Bayesian test of deficit with covariates | \code{BTD_cov()} | @crawford_comparing_2011 | Section \@ref(cov)  -->
<!-- Unstandardised difference test | \code{UDT()} | @crawford_testing_2005 | Equation \@ref(eq:UDT) -->
<!-- Revised standardised difference test | \code{RSDT()} | @crawford_testing_2005 | Equation \@ref(eq:RSDT) -->
<!-- Bayesian standardised difference test | \code{BSDT()}| @crawford_comparison_2007 | Section \@ref(BSDT) -->
<!-- Bayesian standardised difference test with covariates | \code{BSDT_cov()}|  @crawford_comparing_2011 | Section \@ref(cov)  -->

<!-- To facilitate meta-analyses of studies using case-control comparisons -->
<!-- all of the above functions can take both summary statistics as -->
<!-- input as well as raw data. In any case, begin by installing and loading -->
<!-- the package: -->
...
provided it is suggested that the DF's experience of the illusion is unaffected
...
recommends however to only include a covariate if it correlates \(>0.3\) with
...
without inflating effect sizes should be used. In the frequentist framework the
...
The Bayesian analogue of this test is also appropriate to use and is in fact
recommended over the RSDT \citet{crawfordComparisonSingleCase2007}, as described in Section \ref{BSDT}.
The syntax of the two functions is
similar, but the BSDT comes with more optional arguments. For example, one has
the option of applying this test without standardising the variates by setting
the argument \code{unstandardised} to \code{TRUE}. The output then converge to
that of the frequentist UDT. Furthermore, one has the option of choosing
a prior distribution. Setting the argument \code{calibrated} to
\code{FALSE} specifies the use of the ``standard theory'' prior. If left
to the default (\code{TRUE}) an accept-reject algorithm is deployed for each
simulation of \(\Sigma\), as described in Section \ref{BSDT}. This default
prior has been shown to have better frequentist properties for estimating \(\rho\)
and differences between means in a bivariate normal distributions
\citep{bergerObjectivePriorsBivariate2008} and is therefore the default and recommended choice.
```{r BSDT}
...
In recent work by the authors \citep{mcintoshPowerCalculationsSinglecase2020}, we argued that,
...
use the function names with \code{_power} added as a suffix.

So, for example, to calculate power for the test of deficit we call \code{TD_power()}.
Expected case score and either sample size or desired power must
...
in standard deviations. An \(\alpha\)-level of the conventional
\(0.05\) is assumed if nothing else is supplied. The alternative
hypothesis can also be specified to the argument \code{alternative}, for a one-tailed test
specify \code{"less"} (default) or \code{"greater"}, for a two-tailed \code{"two.sided"}.
```{r TD-power}
TD_power(case = 70,
         mean = 100,
         sd = 15,
...
desired level of power. If we for example specify a desired power level of 0.6,
...
power will not increase more than 0.5\% for any additional patient after a sample
size of 15. That is, the algorithm stops searching
when this level of specificity has been reached and we are nearing the
asymptotic maximum power for this effect size. We can increase this specificity
...
is, however, relatively similar to that of \code{TD_power()}. For \code{BTD_power()}
...
of 0 and a standard deviation of 1. The correlation matrix is specified as a \(3\times 3\)
identity matrix. This function is computationally intense and hence, the number
...
\code{BSDT_power()} and \code{UDT_power()} additionally requires an estimate of
...
The \pkg{singcar} package for \proglang{R} has in this paper been outlined and
...
by a small sample. Methods for estimating abnormality for a single variate are
...
are frequently used, especially the test of deficit, Equation \eqref{eq:TD}, and the revised
standardised difference test, Equation \eqref{eq:RSDT}. However, the more
flexible methods allowing for the inclusion of covariates have not gained as
much traction. It is hoped that by providing them in a documented package for a
popular language such as \proglang{R}, these methods will receive further spread.

The methods described has been developed for keeping transparent control over
Type I errors but to emphasise the importance of Type II errors in the paradigm
of case-control comparisons, power calculators have been implemented in the
