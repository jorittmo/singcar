...
test_that("errors are occuring as they should", {
  ...
  expect_error(BTD(-2, controls = 0, controls.sd = 1), "Input sample size")

  expect_error(BTD(rnorm(2), controls, na.rm = TRUE), "Case should only have 1 observation")

  expect_error(BTD(2, controls, na.rm = TRUE, int.level = 2), "Interval level must be between 0 and 1")
...
test_that("TD and BTD gives equal output", {
  ...
  p_bdt <- BTD(-2, controls = 0, controls.sd = 1, controls.n = 15)[["p.value"]]
  p_td <- TD(-2, controls = 0, controls.sd = 1, controls.n = 15)[["p.value"]]

  expect_equal(round(p_bdt, 2), round(p_td, 2))

  p_bdt <- BTD(-2, controls = 0, controls.sd = 1, controls.n = 15, alternative = "g")[["p.value"]]
  p_td <- TD(-2, controls = 0, controls.sd = 1, controls.n = 15, alternative = "g")[["p.value"]]
