...
test_that("errors and warnings are occuring as they should", {
  ...
  expect_error(UDT(-2, -4, rnorm(20), 0, controls.y.sd = 1), "Please set sample size")

  expect_error(UDT(-2, -4, rnorm(20), 0, controls.n = 20),
               "Please give sd and n on task y if controls.y is to be treated as mean")

  expect_error(UDT(-2, -4, rnorm(20), 0, controls.n = 20),
               "Please give sd and n on task y if controls.y is to be treated as mean")

  expect_message(UDT(-2, -4, rnorm(20), rnorm(20), controls.x.sd = 1),
                 "Value on controls.x.sd will be ignored")

  expect_message(UDT(-2, -4, rnorm(20), rnorm(20), controls.y.sd = 1),
                 "Value on controls.y.sd will be ignored")

  expect_message(UDT(-2, -4, rnorm(20), rnorm(20), controls.n = 15),
